...
import {
  ...
  SidebarFooter,
  SidebarInset,
  // SidebarTrigger, // No longer used here as header is removed
...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  if (!user && !publicPaths.includes(pathname)) {
    // For public paths when not logged in, or during initial auth check before user is confirmed null
    return (
      <main className={cn(useSpecialBackground ? "bg-[#4f6749]" : "bg-background")}>
        {children}
      </main>
    );
  }
  ...
  return (
    <SidebarProvider defaultOpen={false}> {/* Changed defaultOpen to false */}
      ...
      {mounted && isMobile && user && <MobileBottomNav />}
    </SidebarProvider>
  );
}
